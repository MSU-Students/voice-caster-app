class MicrophoneSettingService {
  async devices() {
    if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) {
      throw Error(
        "UserMediaDetector getDevices failed: enumerateDevices is not supported"
      );
    }
  ...
  async permitted(kind) {
    if (
      !kind ||
      !Object.values(MicrophoneSettingService.Kinds).includes(kind)
    ) {
      throw Error(
  ...
  async permittedAll(kinds = Object.values(MicrophoneSettingService.Kinds)) {
    ...
    const permissionStates = await Promise.all(
      kindsArray.map(kind => this.permitted(kind))
    );
